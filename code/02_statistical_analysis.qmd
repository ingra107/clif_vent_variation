...
```{r}
##########################
### BINARY Height Interaction RCS ##
#########################
if (hosp_id_count == 1) {
  # Fit the mixed-effects model with random intercepts for patients and hospitals, including pH and pCO2
  mixed_model_sex_interaction_height_ltvv_rcs_bin <- glmer(
    htvv ~
      sex_category * rcs(height_inches) +
      age_at_admission +
      laps2 +
...
# Tidy the mixed model results
model_results <- tidy(mixed_model_sex_interaction_height_ltvv_rcs_bin, effects = "fixed", conf.int = TRUE)
print(model_results)

# Select relevant columns and rename them
results_sex_interaction_height_ltvv_rcs_bin <- model_results %>%
  dplyr::select(
    Variable = term, Estimate = estimate, StdError = std.error, 
  ...
  # Add StudyID and Model columns
  mutate(
    StudyID = "sex_interaction_height_ltvv_rcs_bin",  
    Model = "MixedEffectsModel",
    Site = site_name
  ) %>%
  # Rearrange columns
...
write.csv(results_sex_interaction_height_ltvv_rcs_bin, paste0("output/final/model_coefs_sex_interaction_height_RCS_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)
```
...
predictions_model5 <- ggpredict(mixed_model_sex_interaction_height_ltvv_rcs_bin, terms = c("height_inches [all]", "sex_category"))
predictions_model5$Model <- "Model 5: htvv ~ Sex Category * rcs(Height) ~ HTVV (Binary)"
...
ggplot(all_predictions, aes(x = height_inches, y = predicted, color = sex_category)) +
  geom_line(size = 1) +
  facet_wrap(~ Model) +
  labs(

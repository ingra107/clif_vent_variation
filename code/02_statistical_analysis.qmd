...
## Binary HTVV with Sex/Height Interaction RCS
```{r}
...
if (hosp_id_count == 1) {
  # Fit the mixed-effects model with random intercepts for patients and hospitals, including pH and pCO2
  mixed_model_sex_interaction_height_ltvv_rcs_bin <- glm(
    htvv ~
      sex_category * rcs(height_inches) +
      age_at_admission +
      laps2 +
...
# Ensure Model is a factor
predictions_model5 <- all_predictions |> 
  filter(Model == "Model 5: htvv ~ Sex Category * rcs(Height) ~ HTVV (Binary)") |> 
  mutate(Model = factor(Model)) |> 
  mutate(sex_category = group) |> 
  mutate(height_inches = x)

# Plot all models together
ggplot(predictions_model5, aes(x = height_inches, y = predicted, color = sex_category)) +
  geom_line(size = 1) +
  # facet_wrap(~ Model) +
  labs(

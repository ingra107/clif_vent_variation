...
sbt <- df_hourly_resp_support |> 
  mutate(sbt = fifelse(mode_category == "pressure support/cpap", 
                       1, 0)) |>
  group_by(clif_hospitalizations_joined_id, recorded_date, recorded_hour, sbt) |> 
  ...
  ungroup() |> 
  arrange(clif_hospitalizations_joined_id, recorded_date) |> 
  distinct(clif_hospitalizations_joined_id, recorded_date, sbt, pressure_support_set, peep_set)  |> 
  mutate(
    pressure_support_set = fifelse(sbt == 0, NA_real_, pressure_support_set),
    peep_set = fifelse(sbt == 0, NA_real_, peep_set)
  ) |> 
  group_by(clif_hospitalizations_joined_id) |> 
  mutate(hospital_day = row_number()) |> 
  mutate(health_system = site_name) |> 
  ungroup() |> 
  count(health_system, hospital_day, sbt, pressure_support_set, peep_set)
...
write.csv(results_to_save, paste0("output/final/model_coefs_sex_category_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)
```
...
write.csv(results_to_save, paste0("output/final/model_coefs_sex_category_height_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)
```
...
write.csv(results_to_save, paste0("output/final/model_coefs_sex_interaction_height_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)
```
...
if (hosp_id_count == 1) {
  ...
  mixed_model <- lmer(
    vt_per_kg_ibw ~
      sex_category * rcs(height_inches) +
      age_at_admission +
      laps2 +
      # race_category +
      pf_ratio +
...
write.csv(results_to_save, paste0("output/final/model_coefs_sex_interaction_height_RCS_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)
```
...
new_height <- seq(min(htvv_analysis$height_inches), max(htvv_analysis$height_inches), length.out = 100)
...
write.csv(new_data, paste0("output/final/model_preds_sex_category_height_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)


# Plot the predictions
ggplot(new_data, aes(x = height_inches, y = predicted_vt_per_kg_ibw, color = sex_category)) +
  geom_line() +
  labs(
    x = "Height (in)",

...
first_intubation_data <- df_oxygenation_index_analysis_prep |>
  ...
  left_join(df_laps2_scores |> dplyr::select(clif_hospitalizations_joined_id, recorded_date, laps2) |> distinct()) |> 
...
run_analysis_for_variable_finalfit <- function(var_name, interaction_term = FALSE) {
  ...
  tidy_fit <- tidy_fit %>%
    ...
    write.csv(tidy_fit, file = paste0("output/final/model_coefficients__", var_name, "__", suffix, "__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)
...
intubation_data <- df_oxygenation_index_analysis_prep1 |> 
  ...
  mutate(hour_sequence_from_first_vent_to_discharge = 
    ...
    # Add 'day' variable (starting from 1)
...
run_daily_analysis_for_variable_finalfit <- function(var_name, agg_func, interaction_term = FALSE) {
  ...
  var_daily <- var_daily %>%
    group_by(clif_hospitalizations_joined_id) %>%
    arrange(day) %>%
    mutate(!!delta_var_name := !!sym(var_daily_name) - lag(!!sym(var_daily_name))) %>%
    ungroup()
  ...
  explanatory <- c(worst_24hr_name, delta_var_name, "age_at_admission", "sex_category", "race_category", "day", "htvv_daily_percent")
  ...
  explanatory_lr <- explanatory
  form_lr <- paste(dependent, "~", paste(explanatory_lr, collapse = " + "))
  fit <- glm(formula = as.formula(form_lr), data = model_data, family = binomial(link = "logit"))
  ...
  coefficients_filename <- paste0("output/final/model_coefficients_daily__", var_name, "__", suffix, "__", site_name, "_", Sys.Date(), ".csv")
  ...
  explanatory <- c(worst_24hr_name, delta_var_name, "age_at_admission", "sex_category", "race_category", "htvv_daily_percent")
  ...
  explanatory_lr <- explanatory
  form_lr <- paste(dependent, "~", paste(explanatory_lr, collapse = " + "))
  fit <- MASS::glm.nb(formula = as.formula(form_lr), data = model_data)
  ...
  # Generate a regression table using tbl_regression
  tbl <- model_data |> 
    with(MASS::glm.nb(formula = as.formula(form_lr))) %>%
    tbl_regression(exponentiate = TRUE) %>%
    bold_labels() %>%
    italicize_levels() %>%
    bold_p() %>%
  ...
  gtsave(as_gt(tbl), filename = table_filename)
  ...
  plot_filename <- paste0("output/intermediate/coef_plot_vfds__", var_name, "_", suffix, ".png")
  ...
  # Save the coefficients to a CSV file
  coefficients_filename <- paste0("output/final/model_coefficients_vfds__", var_name, "__", suffix, "__", site_name, "_", Sys.Date(), ".csv")
  write.csv(tidy_fit, file = coefficients_filename, row.names = FALSE)
...
if ("mean_airway_pressure_obs" %in% names(intubation_data)) {
...
htvv_analysis <- htvv_step1 %>%
  ...
  left_join(clif_height |> dplyr::select(clif_hospitalizations_joined_id, height_cm))
...
write.csv(results_to_save, paste0("output/final/sex_category_ltvv__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)

# Create a sequence of height values for prediction
new_height <- seq(min(htvv_analysis$height_cm), max(htvv_analysis$height_cm), length.out = 100)
...
# Predict based on fixed effects, ignoring random effects
new_data$predicted_vt_per_kg_ibw <- predict(mixed_model, newdata = new_data, re.form = NA)

# Plot the predictions
ggplot(new_data, aes(x = height_cm, y = predicted_vt_per_kg_ibw, color = sex_category)) +
  geom_line() +
  labs(
    x = "Height (cm)",

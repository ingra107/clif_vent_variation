...
# 
# # renv::install(c("tidyverse", "ggthemes", "systemfonts",  "styler", "readxl", "writexl", "DBI", "dbplyr", "knitr", "pandoc", "janitor", "data.table", "duckdb" ,"powerjoin", "collapse", "tidyfast", "datapasta", "fst", "dtplyr", "bit64", "zoo", "fuzzyjoin", "arrow", "hrbrthemes", "here", "table1", "rvest", "tidymodels", "pscl", "survminer", "gt", "gtsummary", "broom.helpers", "broom.mixed", "lme4", ""lmerTest"))
# 
#  
# 
...
clif_hospitalization_start <- 
  ...
  select(hospitalization_id, admission_dttm, discharge_dttm, age_at_admission, patient_id, 
         # admission_type_name, 
...
clif_adt_hospital <-  clif_adt |>
  mutate(location_category = factor(location_category,
                                    levels = (c("icu", "stepdown", "ward", "er", "procedural", "other")),                                    ordered = TRUE)) |> 
  group_by(clif_hospitalizations_joined_id) |> 
...
df_encounter_laps2_temp1 <- clif_hospitalization |> 
  ...
  mutate(
    ...
    # ED binary variable
    ed_admit_01               = fifelse(str_detect(first_location_category, "er"), 1, 0, 0),
...
##Oxygenation index
```{r}
df_oxygenation_index_analysis_prep2 <- df_hourly_resp_support |>
...
hospital_order <- df_oxygenation_index_analysis_prep %>%
...
first_intubation_data <- df_oxygenation_index_analysis_prep |>
  ...
  left_join(df_oxygenation_index |> select(clif_hospitalizations_joined_id, recorded_date, recorded_hour, oxygenation_index) |> distinct()) |>
  left_join(df_pf_ratio |> select(clif_hospitalizations_joined_id, recorded_date, recorded_hour, pf_ratio) |> distinct()) |>
  left_join(df_sf_ratio |> select(clif_hospitalizations_joined_id, recorded_date, recorded_hour, sf_ratio) |> distinct()) |>
  left_join(df_laps2_scores |> select(clif_hospitalizations_joined_id, recorded_date, laps2) |> distinct()) |> 
  left_join(clif_hospitalization |> select(clif_hospitalizations_joined_id, age_at_admission, admission_dttm, discharge_dttm
                                           # admission_type_category
                                           ) |> distinct()) |> 
  left_join(hospital_order %>% select(hospital_id, hospital_label), by = "hospital_id") |> 
  select(-hospital_id) |> 
...
run_analysis_for_variable_finalfit <- function(var_name, interaction_term = FALSE) {
  ...
  gtsave(tbl, filename = paste0("output/final/finalfit_", var_name, "_", suffix, "_table_", Sys.Date(), ".html"))
  ...
  ggsave(paste0("output/intermediate/forest_plot_finalfit_", var_name, "_", suffix, ".png"), plot = or_plot)
...
new_data <- data.frame(cumulative_htvv_hours = htvv_levels,
                       ...
                       hospital_id = "Hospital 1",  # Set a specific hospital
...
hist(htvv_analysis$vt_per_kg_ibw, main = "Distribution of vt_per_kg_ibw", xlab = "vt_per_kg_ibw")

# Fit the mixed-effects model with random intercepts for patients and hospitals
mixed_model <- lmer(
  vt_per_kg_ibw ~ sex_category + age_at_admission + laps2 + race_category + pf_ratio +
    (1 | hospital_id) + (1 | patient_id),
  data = htvv_analysis
)
summary(mixed_model)

# Extract fixed effects coefficients with confidence intervals
model_results <- tidy(mixed_model, effects = "fixed", conf.int = TRUE)

# Get the sex difference
sex_difference <- model_results %>%
  filter(term == "sex_categorymale")

print(sex_difference)

# Plot residuals vs. fitted values
plot(resid(mixed_model) ~ fitted(mixed_model), main = "Residuals vs Fitted Values")
...
mixed_model_results <- coef(summary(mixed_model))
conf_intervals <- confint(mixed_model, method = "Wald")

sex_difference_mixed <- data.frame(
  Estimate = mixed_model_results["sex_categorymale", "Estimate"],
  `Std. Error` = mixed_model_results["sex_categorymale", "Std. Error"],
  `Lower CI` = conf_intervals["sex_categorymale", 1],
  `Upper CI` = conf_intervals["sex_categorymale", 2],
  `p-value` = mixed_model_results["sex_categorymale", "Pr(>|t|)"]
)

print(sex_difference_mixed)

...
#  
# 
# renv::install(c("tidyverse", "systemfonts", "readxl", "DBI", "dbplyr", "knitr", "janitor", "data.table", "powerjoin", "collapse", "tidyfast", "datapasta", "fst", "dtplyr", "bit64", "arrow", "hrbrthemes", "here", "table1", "rvest", "tidymodels", "pscl", "survival", "survminer"))
# 
...
#| timeit: true
 ...
 # c("ggthemes","styler","writexl","pandoc","duckdb","zoo","fuzzyjoin")
...
create_directories <- function() {
  ...
  dirs <- c("output", "output/intermediate", "output/final")
...
clif_hospitalization_start <- 
  ...
  select(hospitalization_id, admission_dttm, discharge_dttm, age_at_admission, patient_id, admission_type_name, admission_type_category, discharge_name, discharge_category)
...
clif_demographics_combined <- clif_patient |> 
  left_join(clif_hospitalization) |> select(-hospitalization_id) |> distinct()
...
### A FINAL TABLE
write_parquet(mode_hourly_resp_support_table, paste0("output/final/mode_hourly_resp_support_table_",site_name,".parquet"))
...
trach_excl_early_trach <- trach_variation |> 
  group_by(trach_hospital_id) |> 
  mutate(mean_vent_days_before_trach = median(vent_days_before_trach, na.rm = TRUE)) |> 
  # these are early trach... take them out
  filter(vent_days_before_trach > 3) |>
  # these are ltachs
  filter(mean_vent_days_before_trach >5)

# Create a boxplot
ggplot(trach_excl_early_trach, aes(x = trach_hospital_id, y = vent_days_before_trach)) +
  geom_boxplot() +
...
ltvv_female <- ltvv_variation |> 
  left_join(clif_demographics_combined |> select(sex_category, clif_hospitalizations_joined_id) |> distinct()) |> 
...
if(!any(clif_labs$lab_category == "anion_gap")){
...
sodium_df <- clif_labs %>%
  ...
  select(clif_hospitalizations_joined_id, recorded_dttm, recorded_date, sodium = lab_value_numeric,
         lab_order_dttm, lab_result_dttm)

bicarbonate_df <- clif_labs %>%
  filter(lab_category == "bicarbonate") %>%
  select(clif_hospitalizations_joined_id, recorded_dttm, recorded_date, bicarbonate = lab_value_numeric)

chloride_df <- clif_labs %>%
  filter(lab_category == "chloride") %>%
  select(clif_hospitalizations_joined_id, recorded_dttm, recorded_date, chloride = lab_value_numeric)
...
anion_gap_df <- sodium_df %>%
  ...
  select(clif_hospitalizations_joined_id, recorded_dttm.x, anion_gap, lab_order_dttm, lab_result_dttm, recorded_date)
...
#   bind_rows(df_pre_laps2_duplicates) |> 
  ...
  # get the most recent lab (aka last) based on recorded_dttm
    mutate(
      ...
      bicarbonate_recent        = flast(bicarbonate),
...
first_intubation_data <- df_oxygenation_index_analysis_prep |>
  ...
  select(-sex_category) |> 
  left_join(clif_patient |> select(-hospitalization_id) |> distinct()) |> 
  left_join(df_oxygenation_index |> select(clif_hospitalizations_joined_id, recorded_date, recorded_hour, oxygenation_index, pf_ratio, sf_ratio) |> distinct()) |>
  left_join(df_laps2_scores |> select(clif_hospitalizations_joined_id, recorded_date, laps2) |> distinct()) |> 
  left_join(clif_hospitalization |> select(clif_hospitalizations_joined_id, age_at_admission, admission_type_category)) |> 
...
run_analysis_for_variable <- function(var_name, interaction_term = FALSE) {
  ...
  model_data <- delta_var %>%
    left_join(first_intubation_data %>%
                ...
                summarise(mortality_enc = first(mortality_enc),
                          age_at_admission = first(age_at_admission),
                          sex_category = first(str_to_title(sex_category)),  # Capitalize categories
                          race_category = first(str_to_title(race_category)),  # Capitalize categories
                          ethnicity_category = first(str_to_title(ethnicity_category)),  # Capitalize categories
                          hospital_id = first(hospital_id)),
              by = "clif_hospitalizations_joined_id")
  
  # Model formula (with or without interaction)
  if (interaction_term) {
    formula <- as.formula(paste("mortality_enc ~ worst_24hr * delta_var + 
  ...
  write.csv(model_results, paste0("output/final/logistic_regression_", var_name, "_", suffix, "_results_", Sys.Date(), ".csv"), row.names = FALSE)
  
  # Ensure readable names for terms
  model_results$term <- str_replace_all(model_results$term, "_", " ")  # Replace underscores
  model_results$term <- str_replace(model_results$term, "(?i)sex category", "")  # Remove "category" prefix
  model_results$term <- str_replace(model_results$term, "(?i)race category", "")  # Clean race category
  model_results$term <- str_replace(model_results$term, "(?i)ethnicity category", "")  # Clean ethnicity
  model_results$term <- str_replace(model_results$term, "(?i)hospital id", "")  # Clean hospital
  
  
   # Prepend the variable name to relevant terms
  model_results$term <- ifelse(model_results$term %in% c("Worst 24hr", "Delta Var"), 
                               paste(var_name, model_results$term), 
                               model_results$term)
  ...
  forest_plot <- ggplot(model_results, aes(x = term, y = estimate)) +
    ...
    geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2) +
    coord_flip() +
    labs(title = paste("Forest Plot for", str_to_title(var_name)),
         x = paste("Variables (", str_to_title(var_name), ")", sep = ""),
         y = "Odds Ratio (log scale)") +  # Explicitly define axis labels
    scale_y_log10() +  # Log scale for odds ratios
    theme_minimal() +
    theme(axis.text.y = element_text(size = 10, face = "bold"),  # Make Y-axis text clearer
          axis.title.x = element_text(size = 12, face = "bold"),
          axis.title.y = element_text(size = 12, face = "bold"))
  
  # Show and save the forest plot
  print(forest_plot)
  ggsave(paste0("output/intermediate/forest_plot_", var_name, "_", suffix, ".png"), forest_plot)
...
for (var in variables) {
  # Without interaction
  run_analysis_for_variable(var, interaction_term = FALSE)
  
  # With interaction
  run_analysis_for_variable(var, interaction_term = TRUE)
}
...
## finalfit
```{r}
# Install finalfit package if necessary
...
run_analysis_for_variable_finalfit <- function(var_name, interaction_term = FALSE) {
  ...
  explanatory <- c("worst_24hr", "delta_var", "age_at_admission", "sex_category", 
                   "race_category", "ethnicity_category", "hospital_id")
  
  if (interaction_term) {
    explanatory <- c("worst_24hr*delta_var", explanatory[-c(1, 2)])  # Add interaction term
  }
  
  # Fit the logistic regression model with finalfit
  fit <- model_data %>%
    finalfit(dependent, explanatory, family = "binomial")
  
  # Display the finalfit table
  fit
  
  # Save the finalfit table as a CSV file
  suffix <- ifelse(interaction_term, "interaction", "no_interaction")
  write.csv(fit, paste0("output/final/finalfit_", var_name, "_", suffix, "_results_", Sys.Date(), ".csv"), row.names = FALSE)
  
  # Plot the forest plot
  plot <- finalfit::ff_plot(fit) +
    ggtitle(paste("Forest Plot for", str_to_title(var_name))) +
    theme_minimal()
  
  # Display and save the plot
  print(plot)
  ggsave(paste0("output/intermediate/forest_plot_finalfit_", var_name, "_", suffix, ".png"), plot)
...
## cumulative hours of high tidal volume ventilation

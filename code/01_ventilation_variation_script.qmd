...
# Function to create a sequence of dates between start_date and end_date
hour_sequence_function <- function(start_time, end_time) {
  seq(start_time, end_time, by = "1 hour")
}
...
hour_sequence <- clif_respiratory_support |> 
  ...
  group_by(clif_hospitalizations_joined_id)  |> 
  mutate(first_dttm = fmin(recorded_dttm),
         last_dttm = fmax(recorded_dttm)) |> 
    mutate(recorded_dttms = future_map2(first_dttm, last_dttm, hour_sequence_function)) |> 
    unnest(recorded_dttm = recorded_dttms) 
...
## SBT
```{r}
sbt <- clif_respiratory_support |> 
  filter(mode_category == "pressure support/cpap") |> 
  mutate(sbt = fifelse(mode_category == "pressure support/cpap", 1, 0)) |>
  full_join(hour_sequence) |> 
  ...
  distinct(clif_hospitalizations_joined_id, recorded_date, sbt)  
...
run_analysis_for_variable_finalfit <- function(var_name, interaction_term = FALSE) {
  ...
  tidy_fit <- tidy_fit %>%
    ...
    write.csv(tidy_fit, file = paste0("output/intermediate/model_coefficients__", var_name, "_", suffix, "__", site_name, "_", Sys.Date(), ".csv"), row.names = FALSE)

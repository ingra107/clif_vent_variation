...
### A FINAL TABLE
write.csv(mode_hourly_resp_support_table, paste0("output/final/mode_hourly_resp_support_table_",site_name, "_",Sys.date(),".csv"))
...
<!-- write_csv(trach_variation_table, paste0("output/final/trach_variation_table_",site_name,"_",Sys.Date(),".csv")) -->
...
### A FINAL TABLE
write.csv(ltvv_variation_table, paste0("output/final/ltvv_variation_table_",site_name,"_", Sys.Date(),".csv"))
```
...
write.csv(table_summary, paste0("output/final/hospital_table1_summary_", site_name,"_", Sys.Date(),".csv"), row.names = FALSE)
...
ltvv_female_table <- ltvv_female |> 
  count(hospital_id, sex_category, vt_bin) |> 
  mutate(health_system = site_name)
...
### A FINAL TABLE
write.csv(ltvv_female_table, paste0("output/final/ltvv_female_table_",site_name,"_", Sys.Date(),".csv"))
  



###### I will make this table with everyones data
summary_data <- ltvv_female_table %>%
    filter(hospital_id %in% icu_hosp_ranks_8cc) |> 
  mutate(hospital_id = factor(hospital_id, levels = rev(icu_hosp_ranks_8cc))) |> 
...
ggplot(summary_data, aes(x = proportion, y = hospital_id, fill = vt_bin)) +
...
# Add StudyID and Model columns
  mutate(
    StudyID = "sex_category_ltvv",  # Replace with your actual Study ID

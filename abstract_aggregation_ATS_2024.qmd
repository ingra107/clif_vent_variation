...
combined_model_coefs_daily <- bind_rows(model_objects, .id = "source")

# View the combined dataset
combined_model_coefs_daily

# Get all objects in the environment that start with "model_coefs_sex"
model_objects <- mget(ls(pattern = "^model_coefs_vfds"))

# Combine all objects into one dataset
combined_model_coefs_vfds <- bind_rows(model_objects, .id = "source")

# View the combined dataset
combined_model_coefs_vfds



# Ensure that 'term', 'variable', and 'site' are factors for plotting purposes
model_coefficients <- combined_model_coefs_daily %>%
  mutate(
    term = as.factor(term),
    variable = as.factor(variable),
    site = as.factor(site)
...
ggplot(aes(x = Estimate, y = Site, color = Model)) +
